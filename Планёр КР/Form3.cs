...
namespace Планёр_КР
{
    public partial class Form3 : Form
    {
        ...
        public Form3(int factors, string type)
        {
            InitializeComponent();
            factorCount = factors;
            ...
            this.Text = "Дробный факторный эксперимент (полуреплика)";
        ...
        private void buttonConduct_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                List<ExperimentResult> results = GenerateFractionalFactorial();
        ...
        private List<ExperimentResult> GenerateFractionalFactorial()
        {
            List<ExperimentResult> fullPlan = GenerateFullFactorial();
            List<ExperimentResult> fractionalPlan = new List<ExperimentResult>();
            for (int i = 0; i < fullPlan.Count; i += 2)
            {
                fractionalPlan.Add(fullPlan[i]);
            }
            return fractionalPlan;
        }
