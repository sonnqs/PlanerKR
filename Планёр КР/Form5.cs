...
namespace Планёр_КР
{
    public partial class Form5 : Form
    {
        ...
        public Form5(int factors, string type)
        {
            InitializeComponent();
            factorCount = factors;
        ...
        private void buttonConduct_Click(object sender, EventArgs e)
        {
            try
            {
                if (factorCount != 2)
                {
                    MessageBox.Show("Латинский квадрат поддерживается только для двух факторов.\n" +
                ...
                if (factors[0].Levels != factors[1].Levels)
                {
                    MessageBox.Show("Для латинского квадрата количество уровней обоих факторов должно быть одинаковым.\n" +
                        $"Фактор 1: {factors[0].Levels} уровней\nФактор 2: {factors[1].Levels} уровней",
                        "Ошибка", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                    return;
                }

                List<ExperimentResult> results = GenerateLatinSquare();
        ...
        private List<ExperimentResult> GenerateLatinSquare()
        {
            List<ExperimentResult> results = new List<ExperimentResult>();
            int levels = factors[0].Levels;

            double step1 = (factors[0].MaxValue - factors[0].MinValue) / (levels - 1);
            double step2 = (factors[1].MaxValue - factors[1].MinValue) / (levels - 1);
            ...
            for (int i = 0; i < levels; i++)
            {
                for (int j = 0; j < levels; j++)
                {
                    ...
                    int coded1 = i;
                    int coded2 = (i + j) % levels;

                    result.CodedLevels = new List<int> { coded1, coded2 };

                    double natural1 = factors[0].MinValue + i * step1;
                    double natural2 = factors[1].MinValue + coded2 * step2;

                    result.NaturalValues = new List<double> { Math.Round(natural1, 4), Math.Round(natural2, 4) };
